...
assert(appUpdate.includes("const APP_VERSION = 'v12';"), "앱 업데이트 버전이 v12가 아닙니다.");
...
assert(sw.includes('transfer-english-trainer-v12') && sw.includes('./exam-mode.js') && sw.includes('./grammar-exam.js') && sw.includes('./vocab-exam.js') && sw.includes('./new-problems.js') && sw.includes('./content/vocabulary-02.js') && !sw.includes('./app-ux-parity.js'), "서비스 워커 캐시가 최종 문제 UX 구조와 맞지 않습니다.");
assert(sw.includes('event.data?.type === "SKIP_WAITING"'), "서비스 워커 즉시 업데이트 메시지 처리가 없습니다.");

if (!process.exitCode) {
  console.log("즉시 채점, 어휘 뜻·개별 유의어 출제, 실전 문법 문장형 출제, 모르겠음, 다음 버튼 이동, 다음 문항 위치 정렬, 저장 후 나가기, 앱 업데이트 검증 통과");
}
